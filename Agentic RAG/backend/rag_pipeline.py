...
GRADE_MODEL = os.getenv("GRADE_MODEL", os.getenv("MODEL"))
...
def grade_documents_node(state: RAGState) -> RAGState:
    ...
    try:
        # 🌟 关键修改：在这里强制加入 method="function_calling" 🌟
...
def rewrite_question_node(state: RAGState) -> RAGState:
    ...
    if router:
        ...
        try:
            # 🌟 关键修改：在这里强制加入 method="function_calling" 🌟
            decision = router.with_structured_output(
                RewriteStrategy, 
                method="function_calling"
            ).invoke(
                [{"role": "user", "content": prompt}]
            ...
            # 建议加个打印，方便以后排查其他错误
            print(f"路由决策出错，降级为 step_back。错误信息: {e}") 
            strategy = "step_back"
        # try:
